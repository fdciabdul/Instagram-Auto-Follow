...
/**
 * Start Proxy Server with Existing Config
 */
...
ipcMain.handle('start-bot', async (event, { username, password, mediaShortCode, accountCount = 1, isFirstAccount = false }) => {
    try {
        ...
        const bot = new InstagramBot(
            ...
            path.join(__dirname, 'usernames.db')
        );
      
        await bot.initializeDatabase();

        if (isFirstAccount) {
            // 🥇 First account scrapes usernames
            ...
            // 🗂️ Subsequent accounts retrieve their batch
...
app.whenReady().then(async () => {
    await startProxyServerAtStartup(); // Start ProxyChain on app launch
    createWindow();
});
